     <div class="card">
            <div class="header">
                <h2>REPORTS AND TRENDS</h2>
            </div>
            <div class="body">
            <div class="row">
                <div class="col-md-4">
                <a href="StudioClass-Report-ClassList.php" class="btn bg-blue btn-block waves-effect" role="button" name="btn-walkinlist" data-type="success">CLASS LIST</a>
                 </div>

                 <div class="col-md-4">
                <a href="StudioClass-Report-SessionList.php" class="btn bg-blue btn-block waves-effect" role="button" name="btn-memberlist" data-type="success">SESSION HISTORY</a>
                 </div>

                 <div class="col-md-4">
                <a href="StudioClass-Report-ClassRegistration.php" class="btn bg-blue btn-block waves-effect" role="button" name="btn-clientactivities" data-type="success">CLASS REGISTRATIONS</a>
                 </div>
             </div>
             <div class="row">

                <div class="col-md-4">
                <a href="StudioClass-Trend-MonthlyParticipants.php" class="btn bg-green btn-block waves-effect" role="button" name="btn-signup" data-type="success">MONTHLY CLASS ATTENDEES</a>
                </div>

                <div class="col-md-4">
                <a href="StudioClass-Trend-FaveClass.php" class="btn bg-green btn-block waves-effect" role="button" name="btn-classparticipants" data-type="success">FAVORITE CLASSES</a>
                 </div>

                 <div class="col-md-4">
                <a href="StudioClass-Trend-TopClients.php" class="btn bg-green btn-block waves-effect" role="button" name="btn-signup" data-type="success">TOP PARTICIPANTS</a>
                 </div>

             </div>
             </div>
        </div>